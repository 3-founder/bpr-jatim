        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="main-card mb-3 card">
                    <div class="card-body">
                        <h5 class="card-title">Tambah Jaringan Kantor</h5>
                        <form action="{{ route('jaringan-kantor.store') }}" method="post">
                            @csrf
                                ...
                                <select name="id_kota" id="id_kota"
                                    ...
                                    @foreach ($cabang as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('id_kota') == $item->id ? 'selected' : '' }}>{{ $item->nama_kota }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('id_kota')
                            ...
                            <div class="position-relative form-group">
                                ...
                                <select name="jenis" id="jenis"
                                    ...
                                    <option value="KK" {{old('jenis') == 'KK' ? 'selected' : ''}} >Kantor Kas</option>
                                    <option value="ATM" {{old('jenis') == 'ATM' ? 'selected' : ''}} >ATM</option>
                                    <option value="MK" {{old('jenis') == 'MK' ? 'selected' : ''}} >Mobil Keliling</option>
